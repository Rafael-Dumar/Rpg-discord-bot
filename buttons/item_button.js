...
export default {
    ...
    async execute(interaction) {
        ...
        const potionResult = await pool.query(
            `SELECT inv.inventory_id, i.name, i.description, inv.quantity FROM inventories inv JOIN items i ON inv.item_id = i.item_id WHERE inv.user_id = $1 AND i.type = 'potion' AND inv.quantity > 0 AND i.effect_type NOT IN ('REBIRTH', 'AUTO_REVIVE')`, [userId] 
        );
        ...
        const potionOptions = potionResult.rows.slice(0, 25).filter(potion => potion.effect_type !== 'TRIPLE_TURN').map(potion => ({
