...
export default {
    ...
    async execute(interaction) {
        ...
        try {
            ...
            const itemNameResult = await pool.query('SELECT i.name, i.type FROM inventories inv JOIN items i ON inv.item_id = i.item_id WHERE inv.item_id = $1 AND inv.user_id = $2', [itemId, userId]);
            ...
            // nome do item
            const itemName = itemNameResult.rows[0];
            ...
            if (deleteResult.rowCount === 0) {
                return interaction.reply({content: `Você não está com \`${itemName.name}\` equipado.`, ephemeral: true});
            }
            // se estiver, remove o item da tabela de itens equipados
            const itemtypeEmoji = itemName.type === 'weapon' ? '🗡️' : itemName.type === 'armor' ? '🛡️' : '💍';
            await interaction.reply({content: `✅ Você desequipou ${itemtypeEmoji} **${itemName.name}** com sucesso!`, ephemeral: true});
        } catch (err) {
            console.error(err);
